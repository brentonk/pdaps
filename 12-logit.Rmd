...
This phenomenon is called *complete separation* (terminology from @albert1984existence), and it is a vexing problem for the maximum likelihood estimation of logistic regression models.
...
```{r logit-sep-ex-data, echo=FALSE}
separated_data <-
  tibble(x = seq(-3, 3),
         y = as.numeric(x > 0)) %>%
  select(y, x) %>%
...
```{r logit-sep-curves, echo=FALSE}
beta <- c(0.5, 1, 2, 4, 8)
calc_lik <- function(b) {
  pp <- plogis(separated_data$x * b)
  l <- if_else(separated_data$y == 1, pp, 1 - pp)
  prod(l)
}
lik <- map_dbl(beta, calc_lik)

dat_curves <-
  crossing(beta = beta,
           x = seq(-3, 3, length.out = 101)) %>%
  mutate(y = plogis(x * beta),
         beta = factor(beta))

dat_captions <-
  tibble(beta = as.character(beta),
         x = -1.5,
         y = seq(0.525, 0.975, along = beta),
         caption = str_glue("beta = {beta}, likelihood = {round(lik, 3)}"))

ggplot(dat_curves, aes(x = x, y = y)) +
  geom_line(aes(colour = beta)) +
  geom_text(data = dat_captions,
            aes(label = caption, colour = beta)) +
  geom_point(data = separated_data)
```
...
A closely related but slightly less serious problem is *quasicomplete separation*.
...
Alternatively, @zorn2005solution recommends using the bias-corrected logistic regression estimator developed by @firth1993bias.
What you should *not* do is report whatever numbers your statistical software spits out as if they were proper maximum likelihood estimates --- they are not.
...
This is an example of the *incidental parameters problem*.

The usual fix for the incidental parameters problem is to use the "conditional logit" estimator developed by @chamberlain1980analysis.
...
@beck2018estimating compares these approaches.


### Autocorrelation and Clustered Standard Errors
...
See @huffer1998markov or, for an introduction aimed at political scientists, @ward2002location.
...
These follow the same kind of "sandwich" logic as the formula we derived for OLS [@Liang:1986gv].
In some cases the ordinary estimator may still be consistent, though it will not be efficient.
This requires, very loosely speaking, that the average autocorrelation between observations go to zero as the sample size increases [@poirier1988probit].[^autocorrelated-logit]

[^autocorrelated-logit]: I thank Larry Bartels for drawing my attention to the reference here, in response to a question by a previous PSCI 8357 student.
...
You can build your own maximum likelihood estimators using the **maxLik** package.
...
1.  `beta`: the coefficients to evaluate the log-likelihood at.
2.  `y`: vector of responses (0s and 1s).
3.  `x`: matrix of covariates (same number of rows as length of `y`).

```{r fn-loglik-logit}
loglik_logit <- function(beta, y, x) {
  ## Calculate the expected latent propensity for each observation
  xb <- x %*% beta
  xb <- drop(xb)  # Convert from matrix to vector

  ## Calculate predicted probabilities according to logit model
  pp <- plogis(xb)

  ## Calculate likelihood for each individual observation
  lik <- ifelse(y == 0, 1 - pp, pp)
...
We can use this with the `maxLik()` function to perform maximum likelihood estimation.
We'll construct our `y` and `x` "by hand" from the Iraq vote data.

```{r maxlik-ex-data}
y_iraq <- iraqVote$y
head(y_iraq)
x_iraq <- cbind(intercept = 1,
                republican = as.numeric(iraqVote$rep),
                gorevote = iraqVote$gorevote)
```
...
```{r maxlik-start}
beta_start_iraq <- solve(t(x_iraq) %*% x_iraq, t(x_iraq) %*% y_iraq)
beta_start_iraq <- drop(beta_start_iraq)  # Convert from matrix to vector
beta_start_iraq
```

We can now estimate our hand-rolled logistic regression model.

```{r maxlik-logit-by-hand}
logit_by_hand <- maxLik(logLik = loglik_logit,
                        start = beta_start_iraq,
                        y = y_iraq,
                        x = x_iraq)
summary(logit_by_hand)
```

The coefficient estimates are exactly the same as what `glm()` gave us.
The standard errors are slightly different, though not substantively so, because `glm()` and `maxLik()` use slightly different approximations to obtain standard error estimates.

Of course, you would always want to use `glm()` instead of using your own home-cooked logistic regression estimator.
However, should you ever find yourself needing to estimate a model that hasn't already been baked into R, `maxLik()` is an easy way to do so.
